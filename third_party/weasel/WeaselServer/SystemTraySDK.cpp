...
// system
//                        settings allow it (Matthew Ellis). Updated the
...
//						 Animate() now checks for empty
// icon list - Anton Treskunov 						 Added
// the virutal CustomizeMenu() method - Anton Treskunov
//
...
const UINT CSystemTray::m_nTaskbarCreatedMsg =
...
BOOL CSystemTray::Create(HINSTANCE hInst,
                         HWND hParent,
  ...
  m_hWnd = ::CreateWindow(TRAYICON_CLASS, _T(""), WS_POPUP, CW_USEDEFAULT,
  ...
  // 2007-11-14 GONG
  _tcsncpy_s(m_tnd.szTip, _countof(m_tnd.szTip), szToolTip,
             m_nMaxTooltipLength);
  ...
  if (m_bWin2K && szBalloonTip) {
    ...
    if (szBalloonTitle)
      _tcsncpy_s(m_tnd.szInfoTitle, _countof(m_tnd.szInfoTitle), szBalloonTitle,
                 63);
    else
...
BOOL CSystemTray::SetIcon(LPCTSTR lpszIconName) {
  HICON hIcon = (HICON)::LoadImage(m_hInstance, lpszIconName, IMAGE_ICON, 0, 0,
                                   LR_LOADFROMFILE);
...
BOOL CSystemTray::SetIcon(UINT nIDResource) {
  HICON hIcon = (HICON)::LoadImage(m_hInstance, MAKEINTRESOURCE(nIDResource),
...
BOOL CSystemTray::SetTooltipText(LPCTSTR pszTip) {
  ...
  _tcsncpy_s(m_tnd.szTip, _countof(m_tnd.szTip), pszTip,
             m_nMaxTooltipLength - 1);
...
LPTSTR CSystemTray::GetTooltipText() const {
  ...
  // #ifdef _UNICODE
  //    strBuffer[0] = _T('\0');
  //    MultiByteToWideChar(CP_ACP, 0, m_tnd.szTip, -1, strBuffer,
  //    m_nMaxTooltipLength, NULL, NULL);
  // #else
  //    strncpy(strBuffer, m_tnd.szTip, m_nMaxTooltipLength-1);
  // #endif
  _tcsncpy_s(strBuffer, _countof(strBuffer), m_tnd.szTip,
             m_nMaxTooltipLength - 1);
...
BOOL CSystemTray::ShowBalloon(LPCTSTR szText,
  ...
  // dwBalloonIcon must be valid.
  ASSERT(NIIF_NONE == dwIcon || NIIF_INFO == dwIcon || NIIF_WARNING == dwIcon ||
         NIIF_ERROR == dwIcon);
...
LRESULT PASCAL CSystemTray::WindowProc(HWND hWnd,
                                       UINT message,
                                       WPARAM wParam,
                                       LPARAM lParam) {
  // The option here is to maintain a list of all TrayIcon windows,
...
// Check to see if the animation has been disabled (Matthew Ellis
// <m.t.ellis@bigfoot.com>)
...
BOOL CSystemTray::RemoveTaskbarIcon(HWND hWnd) {
  ...
  if (!::IsWindow(m_hWndInvisible)) {
    // 2007-11-14 GONG
    m_hWndInvisible = CreateWindowEx(
        0, _T("Static"), _T(""), WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT,
        CW_USEDEFAULT, CW_USEDEFAULT, NULL, 0, NULL, 0);
